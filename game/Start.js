...
var ctx;
var loadcount = 0;
var Count = 18; // 몇 개의 이미지를 만들지 카운터 객수 배열보다 하나 더 0부터 시작해서..
...
var arMeteEffect =  new Array(); // 운석 폭발 이펙트

var ScoreImage = new Array(); // 점수 이미지.




var MouseClick = {}; MouseClick.x = 0 ,MouseClick.y = 0;

var Over;

var DateScore = 0; // 데이터 전송 및 점수.

var Date = {};
Date.score;
Date.player_id;
// Date.api_key = 'b01k827azijak2ur30fa';
Date.content;
var score;
...
{	
	...
	img[15].src = "Image/Start/UI/숫자.png";
	img[16].src = "Image/Start/UI/게임오버.png";
	img[17].src = "Image/Start/UI/게임오버버튼.png";
...
function OverKey()
{
	...
	window.onclick = function( mc)
	{// 마우스 왼쪽 클릭 시
		Date.player_id = prompt("사용자 이름을 입력하세요.");
		Date.content = prompt("한줄 코멘트를 입력하세요.");
		Date.score = DateScore
		Date.api_key = 'qx5wmuvj9wjfjtlctk94'
		score = JSON.stringify(Date);
		
		console.log(Date.player_id);
		console.log(Date.comment);
		console.log(Date.score);

		var url = "http://golony.duckdns.org/gamedata/sync";
...
{ // 운석 객체
	...
	for ( i = 0 ; i < 4 ; i ++ )
	{
		ScoreImage[i].SpriteMin =FontMove(DateScore, i );
	}
...
function drawMain() 
{
	...
	} else if ( state == PROCESS_End ) {
		ctx.drawImage(img[16], 0,0); // 배경화면 
		ctx.drawImage(img[17], 200,320); //  
	 }else {
	...
	//	console.log(img[2]);
		...
		for ( var i = 0 ; i < 4 ; i ++)
		{
			ctx.drawImage(img[15], Math.floor(ScoreImage[i].SpriteMin % 10) * 20, Math.floor(ScoreImage[i].SpriteMin / 10 ) * 33,
				   	20, 33, ScoreImage[i].x, ScoreImage[i].y, 20, 33);
		}
...
function FontMove( Socre, key ) 
{
	...
	switch(key)
	{
		...
		case 2:
			...
			break;
		case 3:
			ImgNum=(Socre/1000)%10;
			break;
		case 4:
			ImgNum=(Socre/10000)%10;
			break;
		case 5:
			ImgNum=(Socre/100000)%10;
			break;
	}
	return ImgNum;
}
